...
with tab2:
    if df_today is not None and not df_today.empty:
        # Поточні дані для віджета
        now_hour = datetime.now(UA_TZ).hour
        # Безпечний пошук поточної години
        current_data = df_today[df_today['Time'].dt.hour == now_hour].iloc[0] if now_hour < len(df_today) else df_today.iloc[0]
        
        # НОВИЙ БАЛАНС: 1 (інфо) до 2 (графік)
        c1, c2 = st.columns([1.2, 2.5])
        
        with c1:
            # Компактна, але насичена панель
            st.markdown(f"""
                <div style='background: linear-gradient(135deg, #1a1c20, #0f1113); padding: 20px; border-radius: 15px; border: 1px solid #32383e; height: 280px;'>
                    <div style='color: #00d4ff; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;'>Нікополь: Live</div>
                    <div style='display: flex; align-items: baseline; gap: 10px;'>
                        <span style='font-size: 52px; font-weight: 800;'>{current_data['Temp']:.0f}°</span>
                        <span style='color: gray; font-size: 18px;'>{get_weather_icon(current_data['Clouds'], current_data['Rain'])}</span>
                    </div>
                    <hr style='margin: 10px 0; border: 0; border-top: 1px solid rgba(255,255,255,0.1);'>
                    <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 10px;'>
                        <div>
                            <p style='color:gray; font-size:11px; margin:0;'>ХМАРНІСТЬ</p>
                            <p style='font-size:18px; font-weight:bold; margin:0;'>{current_data['Clouds']:.0f}%</p>
                        </div>
                        <div>
                            <p style='color:gray; font-size:11px; margin:0;'>ОПАДИ</p>
                            <p style='font-size:18px; font-weight:bold; margin:0;'>{current_data['Rain']:.1f} мм</p>
                        </div>
                        <div style='grid-column: span 2;'>
                            <p style='color:gray; font-size:11px; margin:0;'>ЕНЕРГІЯ НЕБА</p>
                            <p style='font-size:18px; font-weight:bold; color:#FFD700; margin:0;'>{current_data['Radiation']:.0f} W/m²</p>
                        </div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
        ...
        with c2:
            # Збільшений графік
            st.markdown("<p style='color:gray; font-size:11px; text-align:right; margin-bottom:5px;'>ГРАФІК ІНСОЛЯЦІЇ (W/m²)</p>", unsafe_allow_html=True)
            chart_data = df_today.set_index('Time')[['Radiation']]
            st.area_chart(chart_data, color="#FFD700", height=280)

        st.markdown("---")

        # ТАЙМЛАЙН ДОБИ (залишаємо, бо він добре виглядає)
        ...
        for i, (idx, row) in enumerate(display_hours.iterrows()):
            with cols[i]:
                st.markdown(f"""
                    <div style='text-align: center; background: rgba(255,255,255,0.03); padding: 10px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);'>
                        <div style='font-size: 11px; color: gray;'>{row['Time'].strftime('%H:%M')}</div>
                        <div style='font-size: 22px; margin: 5px 0;'>{get_weather_icon(row['Clouds'], row['Rain'])}</div>
                        <div style='font-weight: bold; font-size: 16px;'>{row['Temp']:.0f}°</div>
                        <div style='font-size: 10px; color: #FFD700;'>{row['Radiation']:.0f}W</div>
                    </div>
                """, unsafe_allow_html=True)
        
        # ФІНАЛЬНІ МЕТРИКИ (Футер)
        st.markdown("<br>", unsafe_allow_html=True)
        m1, m2, m3 = st.columns(3)
        m1.metric("ПІК СОНЦЯ", f"{df_today['Radiation'].max():.0f} W/m²")
        m2.metric("СЕРЕДНЯ ХМАРНІСТЬ", f"{df_today['Clouds'].mean():.0f}%")
        m3.metric("ВЕРСІЯ СИСТЕМИ", "SkyGrid v4.3 Stable")
