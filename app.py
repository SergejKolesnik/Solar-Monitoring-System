...
# 1. КОНФІГУРАЦІЯ
st.set_page_config(page_title="SkyGrid Solar AI v19.4", layout="wide")
...
try:
    ...
    if len(df_train) > 24:
        ...
        model_acc = 0
        if df_f is not None: df_f['AI_MW'] = df_f['Rad'] * 11.4 * 0.001
except:
    df_h, model_acc = pd.DataFrame(), 0

# ІНТЕРФЕЙС
st.title("☀️ SkyGrid Solar AI v19.4")
...
with t1:
    # Головний екран: Тільки чиста аналітика
...
with t3:
    # Сюди перенесено всі попередження та технічну інформацію
    st.subheader(f"Статус системи та точність: {model_acc:.1f}%")
    
    # Попередження про затримку даних АСКОЕ
    if not df_h.empty:
        last_t = df_h['Time'].max()
        diff = (now_ua - last_t).total_seconds() / 3600
        if diff > 3:
            st.warning(f"🔔 Зверніть увагу: Дані АСКОЕ затримуються на {int(diff)} год. (Останній запис: {last_t.strftime('%H:%M')})")
    
    if not df_h.empty and 'Fact_MW' in df_h.columns:
        st.write("### 🔥 Теплова карта похибок (Факт - План ШІ)")
        hist = df_h.tail(168).copy()
        # Для теплової карти використовуємо останню навчену модель
        try:
        ...
        except: st.info("Недостатньо даних для побудови карти похибок")

with t4:
    st.write("Останні 100 годин у базі:")
