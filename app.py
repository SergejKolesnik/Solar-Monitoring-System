...
# 1. КОНФІГУРАЦІЯ
st.set_page_config(page_title="SkyGrid: Solar AI v8.0", layout="wide")
...
try:
    ...
    if not df_valid.empty:
        df_recent = df_valid[df_valid['Time'] > (now_ua - timedelta(days=3))]
...
st.markdown(f"""<div class="main-header">
    ...
    <h1 style='margin:0;'>SkyGrid Solar AI <span style='color:#00ff7f; font-size:16px;'>v8.0</span></h1>
...
c1.metric("ОЦІНКА AI (СЬОГОДНІ)", f"{s_ai:.1f} MWh")
c2.metric("ПРОГНОЗ WEATHER API", f"{s_raw:.1f} MWh")
c3.metric("КОЕФІЦІЄНТ КОРЕКЦІЇ", f"{ai_bias:.2f}x")
c4.metric("СТАТУС", "Online", delta="Синхронно")

tab1, tab2 = st.tabs(["📊 АНАЛІЗ ТА ПОРІВНЯННЯ", "🌦 МЕТЕОУМОВИ"])

with tab1:
    st.subheader("📅 Порівняння: Метео (Visual Crossing) vs SkyGrid (AI) vs Факт")
    ...
    fig_h = go.Figure()
    fig_h.add_trace(go.Scatter(x=df_p['Time'], y=df_p['AI_MW'], fill='tozeroy', name="AI План (МВт)", line=dict(color='#00ff7f', width=3)))
    fig_h.add_trace(go.Scatter(x=df_p['Time'], y=df_p['Raw_MW'], name="Метео Прогноз (МВт)", line=dict(color='gray', width=1, dash='dot')))
    
    for date, total_ai in daily_sums_ai.items():
        total_raw = daily_sums_raw[date]
        fig_h.add_annotation(
            x=f"{date} 12:00:00", y=df_p[df_p['Time'].dt.date == date]['AI_MW'].max() + 0.4,
            text=f"<b>AI: {total_ai:.1f} | Метео: {total_raw:.1f} MWh</b>", showarrow=False, font=dict(color="#FFD700", size=12),
    ...
    st.download_button(label="💾 Завантажити прогноз в Excel", data=output.getvalue(), file_name=f"Solar_AI_v8.xlsx")
