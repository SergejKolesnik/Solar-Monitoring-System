...
# КОНФІГУРАЦІЯ
st.set_page_config(page_title="SkyGrid Solar AI v19.0", layout="wide")
...
try:
    ...
    model_acc = r2_score(df_train['Fact_MW'], model.predict(df_train[features].fillna(0))) * 100
    ...
    model, model_acc, df_h = None, 0, pd.DataFrame()

# ІНТЕРФЕЙС
st.title("☀️ SkyGrid Solar AI v19.0")
st.caption(f"АТ «НЗФ» • С.І. Колесник • {now_ua.strftime('%d.%m.%Y %H:%M')}")

t1, t2, t3, t4 = st.tabs(["📊 ПРОГНОЗ", "🌦 МЕТЕОЦЕНТР", "🧠 МОНІТОР НАВЧАННЯ", "📑 БАЗА"])

with t1:
    if not df_h.empty:
        diff = (now_ua - df_h['Time'].max()).total_seconds() / 3600
        if diff > 3: st.error(f"🛑 Дані АСКОЕ затримуються на {int(diff)} год!")
...
with t2:
    ...
    for i, d in enumerate(day_forecast):
        cols[i].markdown(f"<div style='text-align:center; background:#1E2127; padding:10px; border-radius:10px;'><b>{d['Дата']}</b><br>🌡️ {d['Макс']}°</div>", unsafe_allow_html=True)
...
with t3:
    ...
    perf = hist.groupby(hist['Time'].dt.date).agg({'Forecast_MW':'sum', 'AI_MW':'sum', 'Fact_MW':'sum'}).reset_index()
    fig_b = go.Figure()
    ...
    st.write("### 🔥 Теплова карта похибок")
