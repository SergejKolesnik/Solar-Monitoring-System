...
# --- ВКЛАДКА 2: ПРОГНОЗ ПОГОДИ ---
with tab_weather:
    st.subheader("Погодинний прогноз (Сьогодні)")
    
    # Формуємо HTML для горизонтальної стрічки
    weather_html = '<div class="scroll-container">'
    for _, row in df_today.iterrows():
        w_dir = get_wind_dir(row['WindDir'])
        weather_html += f"""
        <div class="weather-card">
            <div style="font-weight: bold; color: #1E3A8A; font-size: 14px;">{row['Time'].strftime('%H:%M')}</div>
            <div style="font-size: 22px; font-weight: bold; margin: 8px 0;">{row['Temp']:.1f}°</div>
            <div style="font-size: 11px; color: #555; line-height: 1.5; text-align: left;">
                ☁️ Хмарність: <b>{row['Clouds']}%</b><br>
                💧 Опади: <b>{row['Rain']} мм</b><br>
                💨 Вітер: <b>{row['WindSp']} м/с {w_dir}</b>
            </div>
        </div>
        """
    weather_html += '</div>'
    
    # Рендеримо горизонтальну стрічку
    ...
    for idx, (date, row) in enumerate(df_10d.iterrows()):
        with d_cols[idx % 5]:
            st.markdown(f"""
            <div style='border: 1px solid rgba(128,128,128,0.2); border-radius:15px; padding:15px; margin-bottom:15px; background: rgba(255,255,255,0.1);'>
                <h4 style='margin:0; color:#1E3A8A; border-bottom: 1px solid #eee; padding-bottom:5px;'>{date.strftime('%d.%m')}</h4>
                ...
                <div style='font-size:12px; line-height:1.6;'>
                    ...
                    <span style='color:#555;'>☁️ Середня хмарн: {row[('Clouds','mean')]:.0f}%</span><br>
...
# 6. ФУТЕР
st.markdown(f"<div class='footer'>Developed by Sergii Kolesnyk | SkyGrid v3.6.4<br>АТ 'НЗФ' Цех Мереж і Підстанцій © 2026</div>", unsafe_allow_html=True)

# 7. АНАЛІТИКА ТОЧНОСТІ
if df_fact is not None:
    with st.expander("📊 Аналіз точності ШІ (Остання доба)"):
        ...
        fig2.update_layout(height=300, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0,r=0,t=30,b=0))
