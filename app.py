...
# 1. Заголовок малюємо ВІДРАЗУ (щоб сайт не був пустим)
...
if not df_f.empty:
    try:
        # Завантажуємо базу з обходом кешу
        ...
        # --- БЕЗПЕЧНИЙ ВИКЛИК МОЗКУ ---
        try:
            # Модель сама вибере наявні колонки для навчання
            predictions, accuracy, importance, error_history = train_and_get_insights(df_h, df_f)
            df_f['AI_MW'] = predictions
        except Exception as ai_err:
            # Якщо мозок видав помилку — підміняємо дані, щоб графік працював
            df_f['AI_MW'] = df_f['Forecast_MW']
            accuracy, importance, error_history = 0.0, None, None
            st.warning("⚠️ ШІ вчиться на нових даних. Поки використовуємо базовий прогноз.")
        ...
        with tabs[1]:
            if importance is not None:
                ...
                st.info("📊 Аналітика стане детальною, коли автоматика додасть у базу хмарність та вітер.")
    ...
    st.error("❌ Метеосервіс тимчасово недоступний.")
