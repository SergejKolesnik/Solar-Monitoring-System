...
st.set_page_config(page_title="SkyGrid: Solar AI Nikopol v3.8.7", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    .day-temp-max { font-size: 30px; font-weight: 800; color: #ffffff; line-height: 1; }
    .day-temp-min { font-size: 18px; font-weight: 600; color: #aeb6bf; margin-top: 5px; }
...
def get_weather_data():
    ...
    try:
        ...
        if res.status_code == 429: return "429"
        ...
        # Формула розрахунку базової генерації (11.4 МВт встановленої потужності)
...
# 4. ЗАВАНТАЖЕННЯ ТА ОБРОБКА
weather_res = get_weather_data()
if isinstance(weather_res, str):
    if weather_res == "429":
        st.warning("⚠️ Перевищено ліміт запитів до метеослужби (429). Використовуйте дані з минулого сеансу або зачекайте 15 хв.")
    else:
        st.error("📡 Помилка підключення до метеосервера.")
    st.stop()
...
try:
    ...
    # Розрахунок ШІ-корекції (Bias) по останньому повному дню
...
with tab1:
    ...
    fig1.update_layout(height=300, margin=dict(l=10, r=10, t=10, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), legend=dict(orientation="h", y=1.1, x=1))
    st.plotly_chart(fig1, use_container_width=True)

    # Графік порівняння (Навчання ШІ)
    if df_fact is not None:
        st.subheader("📊 Аналіз точності (Останні 3 дні)")
        ...
        fig_learn = go.Figure()
        fig_learn.add_trace(go.Scatter(x=merged['Time'], y=merged['Base_MW']*ai_bias, name="План ШІ", line=dict(color='#00d4ff', width=2, dash='dot')))
