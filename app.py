...
if not df_f.empty:
    try:
        # Завантажуємо вашу оновлену базу
        ...
        try:
            ...
            st.error(f"⚠️ Помилка всередині моделі: {ai_err}")
        ...
        with tabs[1]:
            if importance is not None and not importance.empty:
                ...
                st.warning(f"📉 ШІ ще не бачить достатньо даних. У базі знайдено {len(df_h.dropna(subset=['Fact_MW']))} годин із фактом виробітку. Потрібно мінімум 20.")

        with tabs[2]:
            st.subheader("Останні дані в базі")
            st.dataframe(df_h.tail(24), use_container_width=True)

    except Exception as e:
        st.error(f"❌ Помилка завантаження файлу CSV: {e}")
else:
