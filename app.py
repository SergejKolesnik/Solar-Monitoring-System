...
# 1. КОНФІГУРАЦІЯ
st.set_page_config(page_title="SkyGrid: Solar AI Nikopol v3.7.5", layout="wide", initial_sidebar_state="collapsed")
...
with tab_main:
    if df_all is not None:
        ...
        fig1.update_layout(height=380, margin=dict(l=10, r=10, t=20, b=10), hovermode="x unified", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', legend=dict(orientation="h", y=1.1, x=1))
        st.plotly_chart(fig1, use_container_width=True)

        # ГРАФІК НАВЧАННЯ (ЗВЕДЕННЯ ФАКТ + ПРОГНОЗ + КОРЕКЦІЯ)
        st.markdown("---")
        ...
        if df_fact is not None:
            ...
            # 1. Базовий прогноз (до корекції)
            fig_learn.add_trace(go.Scatter(x=df_hist_base['Time'], y=df_hist_base['Base_MW'], name="Базовий План (без ШІ)", line=dict(color='rgba(255,255,255,0.2)', width=1, dash='dot')))
            
            # 2. Факт АСКОЕ
            fig_learn.add_trace(go.Scatter(x=df_hist_fact['Time'], y=df_hist_fact['Fact_MW'], name="Факт АСКОЕ", line=dict(color='#ff4b4b', width=3)))
            
            # 3. Коректований План (результат роботи ШІ)
            ...
            fig_learn.update_layout(height=400, margin=dict(l=10, r=10, t=20, b=10), hovermode="x unified", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', legend=dict(orientation="h", y=1.1, x=1))
            st.plotly_chart(fig_learn, use_container_width=True)

with tab_weather:
    # Погодна частина залишається без змін (твоя улюблена горизонтальна стрічка)
    ...
    df_10d = df_all.groupby(df_all['Time'].dt.date).agg({'Temp':['min','max'], 'Clouds':'mean'})
