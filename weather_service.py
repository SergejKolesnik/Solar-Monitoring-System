...
def fetch_weather_data():
    """
    Отримує свіжі метеодані через API Visual Crossing.
    """
    try:
        ...
        if res.status_code == 200:
            ...
            # Одразу додаємо розрахунок базового прогнозу (Сайт)
            df['Forecast_MW'] = df['Rad'] * 11.4 * 0.001
            return df
    except Exception as e:
        st.error(f"Помилка сервісу погоди: {e}")
