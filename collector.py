...
def save_ai_forecast(df, model, features):
        """
            Оновлює AI_Forecast_MW на сьогодні + 3 дні вперед.
                Завжди перезаписує (не зберігає старі значення).
                    Нічні години (до 5:00 та після 21:00) = 0.
                        """
    if model is None:
    ...
    if df_to_predict.empty:
                print("⚠️ Немає погодних даних для прогнозу")
                return df
...
def main():
    ...
    if 11 <= now.hour <= 13:
        ...
        print(f"ℹ️ Час {now.hour}:00 — прогноз ШІ оновлюється тільки о 12:00")
