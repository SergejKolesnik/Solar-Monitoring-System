...
def main():
    ...
    try:
        ...
        for num in last_ids:
            ...
            if email.utils.parsedate_to_datetime(msg.get("Date")).replace(tzinfo=None) < (datetime.now() - timedelta(days=10)):
                continue
    ...
    print(f"✅ База ГЕНЕРАЦІЇ оновлена: {len(df_gen)} записів")

    # --- 2. ПАРСИНГ МЕТЕО (САЙТ) ---
    new_wet_rows = []
    try:
    ...
    print(f"✅ База МЕТЕО оновлена: {len(df_wet)} записів")

    # --- 3. ФІНАЛЬНА СКЛЕЙКА (ДЛЯ ШІ) ---
    # Об'єднуємо по колонці Time (Left Join, щоб пріоритет був у МЕТЕО для прогнозу)
    df_final = pd.merge(df_wet, df_gen, on='Time', how='left')
    
    # Обрізаємо за останні 30 днів
    limit_time = datetime.now() - timedelta(days=DAYS_TO_KEEP)
    df_final = df_final[df_final['Time'] >= limit_time]
    
    df_final.to_csv(FINAL_FILE, index=False)
    print(f"💾 ФІНІШ: Фінальна база {FINAL_FILE} готова для навчання.")
